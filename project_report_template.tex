...
\item TF-IDF Vectorization. Text features are transformed using TF-IDF. This converts textual responses into numerical feature vectors that reflect the importance of words within each response relative to the dataset. This transformation allows text data to be used in machine learning models while reducing the impact of non-informative words.
\end{itemize}

\subsection{Data splitting}
The dataset is divided into training and test sets, with 80\% of the data allocated to the training set and 20\% to the test set. Hyperparameter selection is performed using stratified 5-fold cross-validation on the training set. All pre-processing transformations are fitted using only the training data points and subsequently applied to the test set. Since each student contributes three related observations, group-based splitting is applied to ensure that all responses from the same student remain in the same dataset partition, preventing cross-set data leakage.
